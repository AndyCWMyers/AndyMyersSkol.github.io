export default String.raw`function pdfAttention({ bucket, active, allowed }) {
    ...
    function state(item) {
      if (!item.pdfAttention) item.pdfAttention = { total, scrolled: 0, pages: Array(Math.ceil(total / 32)).fill(0) };
    ...
    // Even a 10,000-page document stays below keepalive's body limit at 16 hours.
    return { maxHours: 16, initialize: state, sample, detach() {
      observer?.disconnect();
